...
export default function SculpturesPage() {
  return (
    <main className="overflow-hidden bg-[#090909] text-[#eee9df]">
      <SiteHeader />
      <section className="relative min-h-[94svh]">
        <Image src={asset("img-3546-web.jpg")} alt="Sculpted and painted mask in the artist's studio" fill priority sizes="100vw" className="object-cover" style={{ objectPosition: "center 58%" }} />
      ...
      <section className="bg-[#c7b49d] py-28 text-[#1b1510] md:py-40">
        <div className="mx-auto grid max-w-[1450px] gap-6 px-5 sm:px-8 md:grid-cols-3">
          {[
            ["img-3545-web.jpg", "Profile and silhouette"],
            ["img-3546-web.jpg", "Painted surface"],
            ["img-3553-snapseedcopy-web.jpg", "Finished character"],
          ].map(([file, label], index) => (
            <figure key={file} className={`${index === 1 ? "md:mt-20" : ""}`}>
              ...
              <figcaption className="mt-4 flex justify-between font-mono text-[7px] uppercase tracking-[0.18em] text-black/38">
                <span>View 0{index + 1}</span><span>{label}</span>
              </figcaption>
            </figure>
          ))}
        </div>
